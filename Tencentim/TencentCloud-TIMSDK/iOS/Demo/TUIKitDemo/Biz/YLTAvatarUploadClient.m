...
@implementation YLTAvatarUploadClient

+ (void)uploadAvatarImage:(UIImage *)image callback:(void (^)(BOOL, NSString *, NSString *))callback {
    if (!callback || !image) {
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        NSArray *urls = [YLTRuntimeApiBase endpointCandidates:@"avatar.php"];
        ...
        for (NSString *urlStr in urls) {
            ...
            NSString *avatarUrl = o[@"url"] ?: o[@"avatarUrl"] ?: o[@"faceUrl"];
            if (avatarUrl.length) {
                dispatch_async(dispatch_get_main_queue(), ^{ callback(YES, avatarUrl, nil); });
                return;
            }
