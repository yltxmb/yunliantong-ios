...
@property (nonatomic, weak) TUITabBarItem *discoverTabItem;
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [YLTUserSettingsHelper applyNotificationPreferences];
    [self setupListener];
    [self setupGlobalUI];
    [self setupConfig];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onMomentsBadgeRefresh:) name:YLTMomentsBadgeRefreshNotification object:nil];
...
- (void)tryPreloadMainVC {
    ...
    self.userID = [YLTLoginSession shared].userId ?: [TCLoginModel sharedInstance].userID;
    self.userSig = [YLTLoginSession shared].userSig ?: [TCLoginModel sharedInstance].userSig;
    ...
    @weakify(self)
    [TUILogin login:YLT_SDKAPPID userID:self.userID userSig:self.userSig config:config succ:^{
      @strongify(self)
...
- (void)loginSDK:(NSString *)userID userSig:(NSString *)sig succ:(TSucc)succ fail:(TFail)fail {
    self.userID = userID;
    self.userSig = sig;
    [TUILogin login:YLT_SDKAPPID userID:self.userID userSig:self.userSig config:self.loginConfig succ:^{
...
- (void)tryAutoLogin {
    [[YLTLoginSession shared] loadFromDefaults];
    NSString *uid = [YLTLoginSession shared].userId ?: [TCLoginModel sharedInstance].userID;
    NSString *sig = [YLTLoginSession shared].userSig ?: [TCLoginModel sharedInstance].userSig;
    [self loginSDK:uid userSig:sig succ:nil fail:nil];
}

- (void)applicationDidBecomeActive:(UIApplication *)application {
    [YLTMomentsNotificationHelper refreshDiscoverBadgeAsync];
    if ([YLTMaintenanceGate isMaintenanceFromCache]) {
        [self kickUserForMaintenanceMode];
        return;
    }
    [YLTPublicConfigRepository refreshInBackground:^(BOOL ok) {
        if ([YLTMaintenanceGate isMaintenanceFromCache]) {
            [self kickUserForMaintenanceMode];
            return;
        }
        ...
        if ([root isKindOfClass:UITabBarController.class]) {
            [YLTAnnouncementHelper maybeShowFrom:root];
        }
    }];
}

- (void)kickUserForMaintenanceMode {
    if (![TUILogin isUserLogined]) {
        [self navigateToMaintenanceAfterKick:[YLTMaintenanceGate maintenanceMessageFromCache]];
        return;
    }
    NSString *msg = [YLTMaintenanceGate maintenanceMessageFromCache];
    [TUILogin logout:^{
        [self navigateToMaintenanceAfterKick:msg];
    } fail:^(int code, NSString *desc) {
        [self navigateToMaintenanceAfterKick:msg];
    }];
}

- (void)navigateToMaintenanceAfterKick:(NSString *)msg {
...
- (void)onMomentsBadgeRefresh:(NSNotification *)note {
    NSInteger total = [note.userInfo[@"total"] integerValue];
    TUITabBarItem *item = self.discoverTabItem;
    if (!item.badgeView) {
        return;
    }
    if (total <= 0) {
        item.badgeView.title = nil;
    } else {
        item.badgeView.title = total > 99 ? @"99+" : [NSString stringWithFormat:@"%ld", (long)total];
    }
}
...
- (UITabBarController *)getMainController_Classic {
    ...
    TUITabBarItem *discoverItem = [[TUITabBarItem alloc] init];
    discoverItem.title = @"发现";
    discoverItem.identity = @"discoverItem";
    discoverItem.selectedImage = TUIDemoDynamicImage(@"tab_discover_selected_img", [UIImage imageNamed:@"contact_selected"]);
    discoverItem.normalImage = TUIDemoDynamicImage(@"tab_discover_normal_img", [UIImage imageNamed:@"contact_normal"]);
    TUINavigationController *discoverNav = [[TUINavigationController alloc] initWithRootViewController:[[YLTDiscoverViewController alloc] init]];
    discoverNav.navigationItemBackArrowImage = backimg;
    discoverItem.controller = discoverNav;
    discoverItem.controller.view.backgroundColor = [UIColor d_colorWithColorLight:TController_Background_Color dark:TController_Background_Color_Dark];
    discoverItem.badgeView = [[TUIBadgeView alloc] init];
    self.discoverTabItem = discoverItem;
    ...
    YLTProfileViewController *setVC = [[YLTProfileViewController alloc] init];
...
- (UITabBarController *)getMainController_Minimalist {
    ...
    discoverItem.title = @"发现";
    discoverItem.identity = @"discoverItem";
    discoverItem.selectedImage = TUIDynamicImage(@"", TUIThemeModuleDemo_Minimalist, [UIImage imageNamed:TUIDemoImagePath_Minimalist(@"contact_selected")]);
    discoverItem.normalImage = TUIDynamicImage(@"", TUIThemeModuleDemo_Minimalist, [UIImage imageNamed:TUIDemoImagePath_Minimalist(@"contact_normal")]);
    TUINavigationController *discoverNav = [[TUINavigationController alloc] initWithRootViewController:[[YLTDiscoverViewController alloc] init]];
    discoverNav.navigationItemBackArrowImage = backimg;
    discoverNav.navigationBackColor = [UIColor whiteColor];
    discoverItem.controller = discoverNav;
    discoverItem.controller.view.backgroundColor = [UIColor d_colorWithColorLight:[UIColor whiteColor] dark:TController_Background_Color_Dark];
    discoverItem.badgeView = [[TUIBadgeView alloc] init];
    self.discoverTabItem = discoverItem;
    [items addObject:discoverItem];
    ...
    YLTProfileViewController *profileVC = [[YLTProfileViewController alloc] init];
    TUINavigationController *setNav = [[TUINavigationController alloc] initWithRootViewController:profileVC];
    setNav.navigationItemBackArrowImage = backimg;
